#!/usr/bin/env python3
"""Create V6 Isaac evaluation contexts without mutating training artifacts."""
...
def moderate_events(events: dict) -> dict:
    ...
    events["add_joint_default_pos"]["params"]["pos_distribution_params"] = [-0.005, 0.005]
    events["base_com"]["params"]["com_range"] = {
        "x": [-0.015, 0.015],
        "y": [-0.02, 0.02],
        "z": [-0.025, 0.025],
    }
    events["randomize_rigid_body_mass"]["params"]["mass_distribution_params"] = [0.9, 1.15]
    return events


def recovery_push_events(events: dict) -> dict:
    events = moderate_events(events)
    events["push_robot"]["interval_range_s"] = [3.7, 5.2]
