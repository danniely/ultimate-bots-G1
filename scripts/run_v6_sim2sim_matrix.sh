...
OUT="${OUT:-$PROJECT/exports/v6/final/mujoco_matrix}"
MODEL_DIR="${MODEL_DIR:-$PROJECT/exports/v6/final/onnx}"
MODEL_STEP="${MODEL_STEP:-000400}"
...
REFERENCE_MOTION="$REFERENCE_ROOT/s_batido_v6_buffered_sonic"
SOURCE_MOTION="$PROJECT/data/source/sonic/s_batido_v6_buffered_sonic"

mkdir -p "$OUT"

# g1_deploy expects the reference argument to contain one or more motion
# subdirectories.  Keep the wrapper directory stable and populate the V6
# motion beneath it when a fresh VM only has the source dataset checkout.
...
cleanup_children() {
  ...
  # The former `command | tee` launch could orphan children when tmux was
  # killed.  These exact process patterns are scoped to this dedicated VM.
...
for run in $(seq -w 1 "$RUNS"); do
  ...
  sleep 2
  "$PYTHON" "$PROJECT/scripts/summarize_v6_sim2sim_matrix.py" "$OUT" >/dev/null 2>&1 || true
  echo "run_$run completed=$completed" | tee -a "$OUT/progress.log"
done

cleanup_children
"$PYTHON" "$PROJECT/scripts/summarize_v6_sim2sim_matrix.py" "$OUT"
